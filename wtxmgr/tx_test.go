...
var (
	...
	TstRecvTx, _                    = cashutil.NewTxFromBytes(TstRecvSerializedTx)
	...
	TstSpendingTx, _           = cashutil.NewTxFromBytes(TstSpendingSerializedTx)
...
func serializeTx(tx *cashutil.Tx) []byte {
...
func TestInsertsCreditsDebitsRollbacks(t *testing.T) {
	...
	dupRecvTx, _ := cashutil.NewTxFromBytes(TstRecvSerializedTx)
	...
	TstDoubleSpendTx := cashutil.NewTx(newDupMsgTx)
	...
	TstSpendingTx := cashutil.NewTx(spendingTx)
	...
	tests := []struct {
		...
		},
		{
			...
			bal: 0,
			unc: cashutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
					Hash:  *TstRecvTx.Hash(),
					Index: 0,
		...
		},
		{
			...
			bal: cashutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
					Hash:  *TstRecvTx.Hash(),
		...
		},
		{
			...
			bal: cashutil.Amount(TstDoubleSpendTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
		...
		},
		{
			...
			bal: 0,
			unc: cashutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
		...
		},
		{
			...
			bal: 0,
			unc: cashutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
					Hash:  *TstSpendingTx.Hash(),
					Index: 0,
		...
		},
		{
			...
			bal: cashutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
					Hash:  *TstSpendingTx.Hash(),
...
func TestFindingSpentCredits(t *testing.T) {
	...
	expectedBal := cashutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value)
	if bal != expectedBal {
...
func TestRemoveUnminedTx(t *testing.T) {
	...
	// balance.
	checkBalance := func(expectedBalance cashutil.Amount,
	...
	checkBalance(cashutil.Amount(initialBalance), false)
	checkBalance(cashutil.Amount(initialBalance), true)
	...
	checkBalance(cashutil.Amount(changeAmount), true)
