...
func (w *Wallet) MakeMultiSigScript(addrs []cashutil.Address, nRequired int) ([]byte, error) {
	pubKeys := make([]*cashutil.AddressPubKey, len(addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*cashutil.AddressScriptHash, error) {
	var p2shAddr *cashutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if waddrmgr.IsError(err, waddrmgr.ErrDuplicateAddress) {
				...
				p2shAddr, _ = cashutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*cashutil.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
