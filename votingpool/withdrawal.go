...
func (s *WithdrawalStatus) Fees() cashutil.Amount {
	return s.fees
}
...
func (o OutBailmentOutpoint) Amount() cashutil.Amount {
	return o.amount
}
...
type withdrawalTx struct {
	...
	// struct field instead of a method so that it can be replaced in tests.
	calculateFee func() cashutil.Amount
}
...
func newWithdrawalTx(setOptions func(tx *withdrawalTx)) *withdrawalTx {
	...
	tx.calculateFee = func() cashutil.Amount {
		return cashutil.Amount(1+tx.calculateSize()/1000) * feeIncrement
	}
	setOptions(tx)
	return tx
}
...
func (tx *withdrawalTx) inputTotal() (total cashutil.Amount) {
...
func (tx *withdrawalTx) outputTotal() (total cashutil.Amount) {
...
func (p *Pool) StartWithdrawal(ns walletdb.ReadWriteBucket, addrmgrNs walletdb.ReadBucket, roundID uint32, requests []OutputRequest,
	...
	txStore *wtxmgr.Store, txmgrNs walletdb.ReadBucket, chainHeight int32, dustThreshold cashutil.Amount) (
...
func (w *withdrawal) finalizeCurrentTx() error {
	...
	for _, txOut := range tx.outputs {
		...
		amtFulfilled := cashutil.Amount(0)
...
func (w *withdrawal) maybeDropRequests() {
	inputAmount := cashutil.Amount(0)
	for _, input := range w.eligibleInputs {
		inputAmount += input.Amount
	}
	outputAmount := cashutil.Amount(0)
...
func (wi *withdrawalInfo) match(requests []OutputRequest, startAddress WithdrawalAddress,
	lastSeriesID uint32, changeStart ChangeAddress, dustThreshold cashutil.Amount) bool {
...
func getWithdrawalStatus(p *Pool, ns, addrmgrNs walletdb.ReadBucket, roundID uint32, requests []OutputRequest,
	startAddress WithdrawalAddress, lastSeriesID uint32, changeStart ChangeAddress,
	dustThreshold cashutil.Amount) (*WithdrawalStatus, error) {
		...
		for inputIdx, input := range tx.inputs {
			...
			for i, pubKey := range pubKeys {
				...
				if privKey != nil {
					...
					sig, err = txscript.RawTxInSignature(msgtx, inputIdx, redeemScript, 0, // TODO:Comfirm amount
						txscript.SigHashAll|txscript.SigHashForkID, ecPrivKey)
					if err != nil {
...
func getRedeemScript(mgr *waddrmgr.Manager, addrmgrNs walletdb.ReadBucket, addr *cashutil.AddressScriptHash) ([]byte, error) {
...
func signMultiSigUTXO(mgr *waddrmgr.Manager, addrmgrNs walletdb.ReadBucket, tx *wire.MsgTx, idx int, pkScript []byte, sigs []RawSig) error {
	...
	redeemScript, err := getRedeemScript(mgr, addrmgrNs, addresses[0].(*cashutil.AddressScriptHash))
	if err != nil {
