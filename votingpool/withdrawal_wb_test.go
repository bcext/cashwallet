...
func TestOutputSplittingNotEnoughInputs(t *testing.T) {
	...
	output1Amount := cashutil.Amount(2)
	output2Amount := cashutil.Amount(3)
...
func TestOutputSplittingOversizeTx(t *testing.T) {
	...
	requestAmount := cashutil.Amount(5)
	bigInput := int64(3)
	smallInput := int64(2)
	...
	if tx1.outputs[0].amount != cashutil.Amount(bigInput) {
	...
	if tx2.outputs[0].amount != cashutil.Amount(smallInput) {
...
func TestRollbackLastOutputMultipleInputsRolledBack(t *testing.T) {
	...
	for i, amount := range []cashutil.Amount{4, 3, 2} {
...
func TestRollbackLastOutputWhenNewOutputAdded(t *testing.T) {
	...
	checkTxChangeAmount(t, firstTx, cashutil.Amount(4))
	...
	checkTxChangeAmount(t, secondTx, cashutil.Amount(3))
}
...
func TestWithdrawalTxAddChange(t *testing.T) {
	...
	tx.calculateFee = TstConstantFee(cashutil.Amount(fee))
...
func TestWithdrawalTxInputTotal(t *testing.T) {
	...
	if tx.inputTotal() != cashutil.Amount(5) {
		t.Fatalf("Wrong total output; got %v, want %v", tx.outputTotal(), cashutil.Amount(5))
	}
}
...
func TestWithdrawalTxOutputTotal(t *testing.T) {
	...
	if tx.outputTotal() != cashutil.Amount(4) {
		t.Fatalf("Wrong total output; got %v, want %v", tx.outputTotal(), cashutil.Amount(4))
	}
}
...
func TestSignMultiSigUTXOPkScriptNotP2SH(t *testing.T) {
	...
	pubKeyHashPkScript, _ := txscript.PayToAddrScript(addr.(*cashutil.AddressPubKeyHash))
	msgtx := tx.toMsgTx()
...
func TestSignMultiSigUTXORedeemScriptNotFound(t *testing.T) {
	...
	pkScript, _ := txscript.PayToAddrScript(addr.(*cashutil.AddressScriptHash))
...
func TestStoreTransactionsWithChangeOutput(t *testing.T) {
	...
	inputTotal := cashutil.Amount(0)
	for _, debit := range txDetails.Debits {
		inputTotal += debit.Amount
	}
	if inputTotal != cashutil.Amount(5e6) {
		t.Fatalf("Unexpected input amount; got %v, want %v", inputTotal, cashutil.Amount(5e6))
	}
...
func checkMsgTxOutputs(t *testing.T, msgtx *wire.MsgTx, requests []OutputRequest) {
	...
	for i, request := range requests {
		...
		gotAmount := cashutil.Amount(txOut.Value)
...
func checkTxChangeAmount(t *testing.T, tx *withdrawalTx, amount cashutil.Amount) {
	if !tx.hasChange() {
...
func checkLastOutputWasSplit(t *testing.T, w *withdrawal, tx *withdrawalTx,
	origAmount, newAmount cashutil.Amount) {
