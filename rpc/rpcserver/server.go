...
//
//   https://github.com/bcext/cashwallet/blob/master/rpc/documentation/api.md
//
// Any API changes must be performed according to the steps listed here:
//
//   https://github.com/bcext/cashwallet/blob/master/rpc/documentation/serverchanges.md
package rpcserver
...
// as well as establishing a RPC connection to a gcash consensus server.
...
func (s *walletServer) ImportPrivateKey(ctx context.Context, req *pb.ImportPrivateKeyRequest) (
	*pb.ImportPrivateKeyResponse, error) {
	...
	wif, err := cashutil.DecodeWIF(req.PrivateKeyWif)
	if err != nil {
...
func (s *walletServer) FundTransaction(ctx context.Context, req *pb.FundTransactionRequest) (
	*pb.FundTransactionResponse, error) {
	...
	var totalAmount cashutil.Amount
	for _, output := range unspentOutputs {
		...
		totalAmount += cashutil.Amount(output.Output.Value)

		if req.TargetAmount != 0 && totalAmount > cashutil.Amount(req.TargetAmount) {
			break
		}
	}

	var changeScript []byte
	if req.IncludeChangeScript && totalAmount > cashutil.Amount(req.TargetAmount) {
...
//   sendrawtransaction, so connection errors to gcash could result in the tx
