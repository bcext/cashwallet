...
var rpcHandlers = map[string]struct {
	...
	"getrawchangeaddress":    {handler: getRawChangeAddress}, // todo understand
	...
	// Reference methods which can't be implemented by cashwallet due to
...
// unsupported by cashwallet due to design differences.
func unsupported(interface{}, *wallet.Wallet) (interface{}, error) {
	return nil, &btcjson.RPCError{
		Code:    -1,
		Message: "Request unsupported by cashwallet",
	}
}
...
func makeMultiSigScript(w *wallet.Wallet, keys []string, nRequired int) ([]byte, error) {
	keysesPrecious := make([]*cashutil.AddressPubKey, len(keys))
	...
	for i, a := range keys {
		...
		default:
			...
			pubKeyAddr, err := cashutil.NewAddressPubKey(
...
func addMultiSigAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	secp256k1Addrs := make([]cashutil.Address, len(cmd.Keys))
...
func createMultiSig(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	address, err := cashutil.NewAddressScriptHash(script, w.ChainParams())
	if err != nil {
...
// information about the current state of cashwallet.
// exist.
func getInfo(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	// Call down to gcash for all of the information in this command known
	// by them.
...
func decodeAddress(s string, params *chaincfg.Params) (cashutil.Address, error) {
	addr, err := cashutil.DecodeAddress(s, params)
	if err != nil {
...
// appear in the blockchain, or any tx that has arrived in the gcash mempool).
...
func importPrivKey(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	wif, err := cashutil.DecodeWIF(cmd.PrivKey)
	if err != nil {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCInvalidAddressOrKey,
...
func getTransaction(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	var (
		debitTotal  cashutil.Amount
		creditTotal cashutil.Amount // Excludes change
		fee         cashutil.Amount
		feeF64      float64
	)
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var outputTotal cashutil.Amount
		for _, output := range details.MsgTx.TxOut {
			outputTotal += cashutil.Amount(output.Value)
		}
...
func help(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	// gcash returns different help messages depending on the kind of
	...
	// wallet itself is a websocket client to gcash.  Therefore, create a
	...
	// texts in a non-internal gcash package.
...
func sendPairs(w *wallet.Wallet, amounts map[string]cashutil.Amount,
	account uint32, minconf int32, feeSatPerKb cashutil.Amount) (string, error) {
...
func sendFrom(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	amt, err := cashutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, err
	}
	pairs := map[string]cashutil.Amount{
		cmd.ToAddress: amt,
	}
...
func sendMany(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	pairs := make(map[string]cashutil.Amount, len(cmd.Amounts))
	for k, v := range cmd.Amounts {
		amt, err := cashutil.NewAmount(v)
		if err != nil {
			return nil, err
		}
		pairs[k] = amt
...
func signRawTransaction(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	for _, rti := range cmdInputs {
		...
		if cmd.PrivKeys != nil && len(*cmd.PrivKeys) != 0 {
			...
			addr, err := cashutil.NewAddressScriptHash(redeemScript,
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
	...
	// querying gcash with getrawtransaction. We queue up a bunch of async
	...
	var keys map[string]*cashutil.WIF
	if cmd.PrivKeys != nil {
		keys = make(map[string]*cashutil.WIF)

		for _, key := range *cmd.PrivKeys {
			wif, err := cashutil.DecodeWIF(key)
			if err != nil {
				return nil, DeserializationError{err}
			}
			...
			addr, err := cashutil.NewAddressPubKey(wif.SerializePubKey(),
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
...
func verifyMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	case *cashutil.AddressPubKeyHash: // ok
		return bytes.Equal(cashutil.Hash160(serializedPubKey), checkAddr.Hash160()[:]), nil
	case *cashutil.AddressPubKey: // ok
